...
var fs = require('fs');
var fsProvider = function (sourcePath) {
    if (fs.existsSync(sourcePath)) {
        var fileData = fs.readFileSync(sourcePath, 'utf8');
...
var sourceProviders = [fsProvider];
...
function mapSourcePosition(cache, position) {
  ...
  if (!sourceMap) {
    for (var i = 0;i <sourceProviders.length;i++) {
        ...
        var result = sourceProviders[i](position.source);
        if (result) {
            sourceMap = {
                url: result.url || position.source,
                map: new SourceMapConsumer(result.map || result)
            };
...
exports.addSourceProvider = function (provider) {
